...
export FiltrarMatriz
export AllSegmentsEnergy
...
# -------------------------------------------------------------------------------------------------------- #
"""
    Sparsity( count_non_zeros::Int64, total_elements_of_A::Int64 ) -> Float64
"""
@inline Sparsity( CNZ::Int64, Total::Int64 ) = 1 - CNZ/Total;

# -------------------------------------------------------------------------------------------------------- #
"""
    Density( W::Vector{Any} ) -> Float64
"""
# using Statistics
@inline Density( W::Vector{Any} ) =
    length( findall( ( length.( W ) ./ length( W ) ) .>= ( mean( length.( W )./length( W ) ) +
        2*std( length.( W )./length( W ) ) ) ) );
...
# using JuliaTools.reverberation
"""
...
"""
"""
# using JuliaTools.FiltroMUAremez
function FiltrarMatriz( data::Matrix{Float64}, Variables::Dict{String, Any} )
    datafilt = copy( data );
    for k = 1:size( data, 1 )
        canal = data[ k, : ];
...
"""
# using Statistics
# using JuliaTools.donoho
# using JuliaTools.SegmentsComplet
function AllSegmentsEnergy( datafilt, NewCh4R, Variables, antes = 25, SegmentDuration = 5, σ = 4 )
    ...
    @time for k = 1:length( NewCh4R )
        ...
        if !isempty( AllTh )
            ...
            Temp02 = AllTh[ temp02 ];
            SegmentosCompletos =
                SegmentsComplet( Temp02, antes, SegmentDuration, SamplingRate, binsize, MUA );
    ...
    return AllSegments, AllThs
end

# -------------------------------------------------------------------------------------------------------- #
"""
    RepairData( data::Matrix{Float64}, Variables::Dict{String, Any}, V::Vector{Float64}, DiscartedChannels::Vector{Int64}, lim01 = 1000, σM2 = 6, σHigh = 10 )
     -> DataReparado, FinalFrames, FinalChannelsUnique
        Several metodologies...removing suden discharges from data sets.

"""
# using JLD
# using Statistics
# using StatsBase
# using JuliaTools.SaturacionPositiva
# using JuliaTools.SaturacionNegativaTemporal
# using JuliaTools.FiltrarMatriz
# using JuliaTools.Thresholding
# using JuliaTools.FillingHolesCrux
# using JuliaTools.Get_Groups
# using JuliaTools.donoho
# using JuliaTools.AllSegmentsEnergy

function RepairData( data::Matrix{Float64}, Variables::Dict{String, Any}, V::Vector{Float64}, DiscartedChannels::Vector{Int64}, lim01 = 1000, σM2 = 6, σHigh = 10 )

    nChs = length( Variables[ "Layout" ] ); # total number of channels ( ... 4096 )
    channels = collect( 1:nChs );

    datafilt = FiltrarMatriz( data, Variables );
    datafilt[ DiscartedChannels, : ] .= 0;

    # Metodo 1.
    W = log.( Int.( V ) ); # para detectar los eventos de saturacion
    Todos, t = Thresholding( W );

    SatChannelsVar = sum( Todos, dims = 1 );
    Temp = findall( vec( SatChannelsVar .<= lim01 ) );
    ...
    grupos, loose = Get_Groups( SatChannelsVarFillend );
    Channels4RepairM1 = vcat( grupos... );
    ...
    while k <= nChs
        if isempty( findall( k .== DiscartedChannels ) )
            thr = σM2*donoho( datafilt[ k, : ] );
            upperframes = findall( datafilt[ k, : ] .> thr );
    ...
    allsatframes = vcat( allframes[ FirstDetection ]...);
    Temporal = countmap( allsatframes );
    canal_virtual = zeros( Int, size( data, 2 ) );
    k = Int.( keys( Temporal ) ); v = Int.( values( Temporal ) );
    for m = 1:length( k )
        canal_virtual[ k[ m ] ] = v[ m ];
    end

    thr = 2*σM2*donoho( canal_virtual ); Finals = findall( canal_virtual .>= thr );

    upstairs = [ ]; downstairs = [ ];
    for k = 1:nChs
        A = σM2*donoho( datafilt[ k, : ] ); B = -σM2*donoho( datafilt[ k, : ] );
        a = findall( datafilt[ k, Finals ] .> A ); b = findall( datafilt[ k, Finals ] .< B );
    ...
    grupos, loose = Get_Groups( SatChannelsThrFillend );
    Channels4RepairM2 = vcat( grupos... );

    NewCh4R = union( Channels4RepairM1, Channels4RepairM2 );
    NewCh4R = FillingHolesCrux( NewCh4R );

    AllSegments, AllThs = AllSegmentsEnergy( datafilt, NewCh4R, Variables );

    nopes = findall( Bool.( length.( AllSegments ) .% 2 ) );
    deleteat!( AllSegments, nopes ); deleteat!( AllThs, nopes ); deleteat!( NewCh4R, nopes );

    nframes = AllSegments[ 1 ][ 1, 2 ] - AllSegments[ 1 ][ 1, 1 ] + 1;
    ...
    NewTh = donoho( CanalVirtual )*σHigh;
    ...
    for k = 1:length( FinalChannelsUnique )
    ...
    return DataReparado, FinalFrames, FinalChannelsUnique
end
